 ...
 * Does NOT run tests - expects reports to already exist in web/www.diyaccounting.co.uk/public/tests/.
 *
 * Usage:
 *   node scripts/generate-compliance-report.js --target URL [--output FILE]
 *
 * Expected report files in web/www.diyaccounting.co.uk/public/tests/:
...
const targetDir = join(projectRoot, "web/www.diyaccounting.co.uk/public/tests");
...
**Status**: ${statusIcon(npmAudit.critical === 0 && npmAudit.high === 0)} ${npmAudit.critical === 0 && npmAudit.high === 0 ? "No critical/high vulnerabilities in production dependencies" : "Critical/high vulnerabilities in production dependencies require attention"}`
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/penetration/npm-audit.json\`"
}
...
**Status**: ${statusIcon(eslint.errors === 0)} ${eslint.errors === 0 ? "No security errors" : "Security errors require attention"}`
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/penetration/eslint-security.txt\`"
}
...
**Status**: ${statusIcon(retire.high === 0)} ${retire.high === 0 ? "No high severity vulnerabilities" : "High severity vulnerabilities require attention"}`
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/penetration/retire.json\`"
}
...
${pa11y.results.map((r) => `| ${r.url.replace(targetUrl, "") || "/"} | ${r.errorCount} |`).join("\n")}`
    ...
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/accessibility/pa11y-report.txt\`"
}
...
${axe.violationDetails.map((v) => `| ${v.id} | ${v.impact} | ${v.description} | ${v.nodes} |`).join("\n")}`
    ...
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/accessibility/axe-results.json\`"
}
...
${axeWcag22.violationDetails.map((v) => `| ${v.id} | ${v.impact} | ${v.description} | ${v.nodes} |`).join("\n")}`
    ...
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/accessibility/axe-wcag22-results.json\`"
}
...
**Status**: ${statusIcon(lighthouse.accessibility >= 90)} ${lighthouse.accessibility >= 90 ? "Accessibility score meets threshold (90%+)" : "Accessibility score below 90% threshold"}`
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/accessibility/lighthouse-results.json\`"
}
...
${textSpacing.failedPages.map((p) => `| ${p.url.replace(targetUrl, "") || "/"} | ${p.clippedCount} |`).join("\n")}`
    ...
    : "Report not found: \`web/www.diyaccounting.co.uk/public/tests/accessibility/text-spacing-results.json\`"
}
...
| Report | Path | Status |
|--------|------|--------|
| npm audit | web/www.diyaccounting.co.uk/public/tests/penetration/npm-audit.json | ${npmAudit.found ? "Found" : "Missing"} |
| ESLint Security | web/www.diyaccounting.co.uk/public/tests/penetration/eslint-security.txt | ${eslint.found ? "Found" : "Missing"} |
| retire.js | web/www.diyaccounting.co.uk/public/tests/penetration/retire.json | ${retire.found ? "Found" : "Missing"} |
| Pa11y | web/www.diyaccounting.co.uk/public/tests/accessibility/pa11y-report.txt | ${pa11y.found ? "Found" : "Missing"} |
| axe-core | web/www.diyaccounting.co.uk/public/tests/accessibility/axe-results.json | ${axe.found ? "Found" : "Missing"} |
| axe-core (WCAG 2.2) | web/www.diyaccounting.co.uk/public/tests/accessibility/axe-wcag22-results.json | ${axeWcag22.found ? "Found" : "Missing"} |
| Lighthouse | web/www.diyaccounting.co.uk/public/tests/accessibility/lighthouse-results.json | ${lighthouse.found ? "Found" : "Missing"} |
| Text Spacing | web/www.diyaccounting.co.uk/public/tests/accessibility/text-spacing-results.json | ${textSpacing.found ? "Found" : "Missing"} |
